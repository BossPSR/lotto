        ...
        <div class="content-header row">
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-left mb-0">จัดการกฏกติกา</h2>
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item active">จัดการกฏกติกา
                                </li>
                            </ol>
                        </div>
                    </div>
                    ...
                    <table class="table data-list-view">
                        ...
                        <tbody>
                            ...
                            if ($player_rules) {
                                $i = 0;
                                $index = -1;
                                foreach ($player_rules as $player_rule) {
                                    $i++;
                                    $index++;
                            ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                            $before = isset($player_rules[($index - 1)]) ? $player_rules[($index - 1)] : null;
                                            $after = isset($player_rules[($index + 1)]) ? $player_rules[($index + 1)] : null;
                                            echo "<div class='row' style='display: flex; flex-flow: row;'>";
                                            echo "<form onsubmit='doSubmit(this)' method='POST'>";
                                            if ($before)
                                                echo "<button name='sort' class='btn btm-md btn-warning pl-1 pr-1'><i class='fa fa-chevron-up'></i></button><input type='hidden' name='from' value='" . $before->sort_order_id . "'><input type='hidden' name='to' value='" . $player_rule->sort_order_id . "'>";
                                            echo '</form>';
                                            echo "<form onsubmit='doSubmit(this)' method='POST'>";
                                            if ($after)
                                                echo "<button name='sort' class='btn btm-md btn-warning pl-1 pr-1'><i class='fa fa-chevron-down'></i></button><input type='hidden' name='from' value='" . $after->sort_order_id . "'><input type='hidden' name='to' value='" . $player_rule->sort_order_id . "'>";
                                            echo '</form>';
                                            echo '</div>';
                                        ...
                                        <td class="text-center">
                                            <span data-toggle="modal" data-target="#edit" data-edit-id="{{$player_rule->id}}"><i class="fa fa-edit" style="font-size: 25px;"></i></span>
                                        </td>
                                    </tr>
                            <?php
                                }
                                ...
                                <div class="modal-dialog modal-dialog-scrollable" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form method="POST" action="/admin/rule_huay" onsubmit="doSubmit(this)" enctype="multipart/form-data" >
                                                ...
                                                <div class="form-group  mb-0">
                                                    <div class="text-right">
                                                        <button type="submit" class="btn btn-primary" name="addPlayerRules">เพิ่มข้อมูล</button>
                                                    </div>
                                                </div>
                                            </form>
                                        </div>
                                            ...
                                            <form method="POST" action="/admin/rule_huay" onsubmit="doSubmit(this)" enctype="multipart/form-data">
                                                ...
                                                <div class="form-group  mb-0">
                                                    <div class="text-right" style="display: flex;flex-direction: row-reverse;">
                                                        <button type="submit" class="btn btn-primary" name="editPlayerRules">บันทึกข้อมูล</button>
                                                        <button class="btn btn-danger" name="deletePlayerRules">ลบ</button>
                                                    </div>
                                                </div>
                                            </form>
                                        </div>
    ...
    $(document).ready(function() {
        ...
        $('#edit').on('show.bs.modal', function(event) {
            ...
            $.ajax({
                    ...
                    data: {
                        _token: csrf_token,
                        'target_secret': '{{md5("get-player_rules")}}',
                        'get-id': id
                    },
                    dataType: 'JSON',
